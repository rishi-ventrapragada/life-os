...
/**
 * Compare the stored "yyyy-mm-dd" string to getTodayIST() (Architecture Law 3).
 * ISO date strings sort lexicographically, so a plain string compare is correct
 * — no new Date() math.
 */
function dueLabel(dueDate: string): "Overdue" | "Due today" | null {
...
export default function TaskCard({ task, onEdit, onDelete }: TaskCardProps) {
  const label = task.dueDate ? dueLabel(task.dueDate) : null;
